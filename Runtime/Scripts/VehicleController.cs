...
public class VehicleController : MonoBehaviour
{
    ...
    {
        //air drag (quadratic)
        rb.AddForce(-airDragCoeff * rb.linearVelocity * rb.linearVelocity.magnitude);

        //downforce (quadratic)
        rb.AddForce(-airDownForceCoeff * rb.linearVelocity.sqrMagnitude * transform.up);

        //tire drag (Linear)
        rb.AddForceAtPosition(-tireDragCoeff * rb.linearVelocity, transform.position);
    ...
    private void AutoSteer()
    {
        ...
        if (Mathf.Abs(yawRate) < 10f)
            rb.linearVelocity = Quaternion.AngleAxis(yawRate * autoSteerAmount, Vector3.up) * rb.linearVelocity;
