...
public class NavigationTrigger : MonoBehaviour {
    ...
    private void Start() {
        if (NetworkManager.Singleton.IsClient && !NetworkManager.Singleton.IsServer &&
            ...
            foreach (ClientDisplay participant in ClientDisplay.Instances) {
    ...
    private void Update() {
        ...
        foreach (var t in ObjectsToTrack) {
            ...
            if (!triggered && m_boxcollider.bounds.Contains(t.position)) {
               ...
               // ConnectionAndSpawning.Instance.GetScenarioManager(). UpdateAllNavigationInstructions(temp); //UpdateAllNavigationInstructions got deprecated??
            }
        }
    }
}